/*--------------------------------------------------------------------------
 ...
 Portions Copyright 2023 gbs2@uw.edu
...
#ifdef DEBUG_PRINTS
	fprintf(stderr, "; died on line %d, %s errno %d\n", line, strerror(err), err);
#endif
	if (fp) {
...
int main(int argc, char **argv)
{
	...
	mysetenv("PATH", "/bin:/usr/bin:/usr/local/bin");
	mysetenv("USER", user);
    mysetenv("TERM", "dumb"); /* GBS 2022/06/16 - Unclear if this is the best way to solve this, but 
                                 it works for the glider application */
